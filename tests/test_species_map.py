...
class ProjectTests(unittest.TestCase):
    ...
    def test_build_project(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(project["phenotype_tip_counts"], {"nitrogen_fixation": 2})
            html = (output / "index.html").read_text(encoding="utf-8")
    ...
    def test_build_project_with_gtdb_taxonomy(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            html = (output / "index.html").read_text(encoding="utf-8")
            self.assertIn('`${formatTaxonomyLabel(rank, node.rankValue)} (1)`', html)
            self.assertIn('domain: "d__"', html)
            self.assertIn('phylum: "p__"', html)
            self.assertIn('class: "c__"', html)
            self.assertIn('order: "o__"', html)
            self.assertIn('family: "f__"', html)
            self.assertIn('genus: "g__"', html)
            self.assertIn('species: "s__"', html)
    ...
    def test_build_project_with_genome_metadata(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('const taxonomyBandRight = showGenomeSizes ? metadataOrigin - 12 : treeBaseWidth', html)
            self.assertIn('width: Math.max(24, taxonomyBandRight - node.x)', html)
            self.assertNotIn('svg.querySelectorAll(".clade-taxonomy-band").forEach', html)
    ...
    def test_build_project_with_existing_eggnog_annotations(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('id="totalMin"', html)
            self.assertIn('id="totalMax"', html)
            self.assertIn('id="branchScopeSelect"', html)
            self.assertIn('id="netChangeSelect"', html)
            self.assertIn('id="phenotypeBranchSelect"', html)
            self.assertIn("function branchMatchesFilter(branch)", html)
            self.assertIn("function branchContextMatchesFilter(branch)", html)
            self.assertIn('branchValueMatchesFilter(branch, "gain")', html)
            self.assertIn('branchValueMatchesFilter(branch, "loss")', html)
            self.assertIn('state.branchFilter.scope === "terminal"', html)
            self.assertIn('state.branchFilter.netChange === "gain"', html)
            self.assertIn('state.branchFilter.phenotypeEvent === "transition"', html)
            ...
            self.assertIn("treePanX: 0, treePanY: 0", html)
            self.assertIn("function applyTreePan()", html)
            self.assertIn("state.treePanX = startPanX + dx", html)
            self.assertIn("state.treePanY = startPanY + dy", html)
            ...
            self.assertIn('$("controlTabFilters").append(sectionTitles[1], $("supportCollapseSelect").closest(".control-row")', html)
            ...
            self.assertIn('id="branchWidth" type="range" min="1" max="8" step="0.5"', html)
            self.assertIn("function applyBranchWidth()", html)
            self.assertIn('stroke-width: ${state.branchWidth}', html)
            self.assertIn('id="branchOpacity" type="range" min="10" max="100" step="5"', html)
            self.assertIn("function applyBranchOpacity()", html)
            self.assertIn('species-map-branch-opacity', html)
            self.assertIn('species-map-controls-position', html)
            self.assertIn('header.addEventListener("pointerdown"', html)
            self.assertIn('header.addEventListener("pointermove"', html)
            self.assertIn('aria-label="Drag controls panel"', html)
            self.assertIn('id="treeFontFamily"', html)
            self.assertIn('species-map-tree-font-family', html)
            self.assertIn('--tree-label-font', html)
            self.assertIn('species-map-taxonomy-label-overrides', html)
            self.assertIn('species-map-tip-label-overrides', html)
            self.assertIn('viewport.addEventListener("dblclick"', html)
            self.assertIn('"data-tip-node": node.id', html)
            self.assertIn('Double-click to rename', html)
            self.assertIn('id="treeLabelEditor"', html)
            self.assertIn('const openLabelEditor =', html)
            self.assertIn('stroke-linecap: square', html)
            self.assertIn('stroke: ${color("--tree-branch-color")}', html)
            self.assertIn('id="treeControlWindow"', html)
            self.assertIn('id="treeControlWindowToggle"', html)
            self.assertIn("function initializeTreeControlWindow()", html)
            self.assertIn('species-map-controls-minimized', html)
            self.assertIn('background: var(--cp-bg)', html)
            self.assertIn('id="taxonomyLabelFontSize" type="range" min="6" max="24"', html)
            self.assertIn('id="shuffleTaxonomyColors"', html)
            self.assertIn('species-map-taxonomy-color-seed', html)
            self.assertIn('crypto.getRandomValues(randomValue)', html)
            self.assertIn("const taxonomyBandOuterRadius = outerRadius +", html)
            self.assertIn('font: 600 ${state.taxonomyLabelFontSize}px ${treeLabelFont}', html)
            self.assertIn("overflow-x: auto", html)
            self.assertIn("minmax(max-content, 1fr)", html)
            ...
            self.assertIn('id="branchLengthSelect"', html)
            self.assertIn('value="cladogram">Cladogram', html)
            self.assertIn('value="phylogram">Phylogram', html)
            self.assertIn('state.branchLengthMode === "phylogram"', html)
            self.assertIn("function assignPhylogeneticDistance", html)
            self.assertIn("node.phylogeneticDistance = parentDistance + branchLength", html)
            self.assertIn("horizontalUnits(node) * depthSpacing", html)
            self.assertIn("horizontalUnits(node) * radialStep", html)
            ...
            self.assertIn('id="treeDownloadFormat"', html)
            self.assertIn('<option value="svg">SVG</option>', html)
            self.assertIn('<option value="png">PNG</option>', html)
            self.assertNotIn('<option value="pdf">PDF</option>', html)
            self.assertIn('id="downloadTree"', html)
            self.assertIn('id="downloadNewick"', html)
            self.assertIn("function downloadCurrentTree()", html)
            self.assertIn("function currentTreeNewick()", html)
            self.assertIn('image/svg+xml;charset=utf-8', html)
            self.assertIn('image/png', html)
            self.assertNotIn('application/pdf', html)
            self.assertIn('text/plain;charset=utf-8', html)
            ...
            self.assertIn('viewport.addEventListener("contextmenu"', html)
            self.assertIn('"data-context-branch": node.branch_id || ""', html)
            ...
            self.assertIn('id="taxonomyPrefixToggle"', html)
            self.assertIn('Hide f__/g__', html)
            self.assertIn('function syncTaxonomyPrefixToggle()', html)
            self.assertIn('state.taxonomyPrefixes = !state.taxonomyPrefixes', html)
            self.assertIn('species-map-taxonomy-prefixes', html)
            self.assertIn('state.taxonomyPrefixes ?', html)
            self.assertIn('const applyTaxonomyPrefixPreference', html)
            self.assertIn('option.value = "family,genus"', html)
            self.assertIn('option.value = "genus,species"', html)
            self.assertIn('option.value = "family,genus,species"', html)
            ...
            self.assertIn('Math.hypot(dx, dy) < 4', html)
            self.assertIn('if (interaction.moved)', html)
            self.assertIn('lastTaxonomyClickKey === interaction.key', html)
            self.assertIn('now - lastTaxonomyClickTime <= 500', html)
            self.assertIn('openTaxonomyLabelEditor(interaction.group, event.clientX, event.clientY)', html)
            self.assertIn('pendingTipLabel = event.target.closest("[data-tip-node]")', html)
            self.assertIn('lastTipClickKey === key && now - lastTipClickTime <= 500', html)
            self.assertIn('openTipLabelEditor(pendingTipLabel, event.clientX, event.clientY)', html)
            ...
            self.assertIn('const taxonomyDisplayLegend = new Map()', html)
            self.assertIn('const taxonomyTipCounts = new Map()', html)
            self.assertIn('right.count - left.count', html)
            self.assertIn('const text = `${baseText} (${number(count)})`', html)
            self.assertIn('state.project.phenotype_tip_counts?.[phenotype]', html)
            self.assertIn('const itemY = y + row * rowHeight', html)
            self.assertIn('data-taxonomy-legend-key', html)
            self.assertIn('species-map-taxonomy-legend-offset', html)
            self.assertIn('openTaxonomyLegendEditor(', html)
            self.assertIn('lastLegendClickKey === interaction.key', html)
            self.assertIn('state.taxonomyLegendOffset = next', html)
            self.assertIn('function expandSvgForLegend(group)', html)
            self.assertIn('group.keepVisible?.()', html)
            self.assertIn('taxonomyLegend.parentNode?.append(taxonomyLegend)', html)
            self.assertIn('data-taxonomy-legend-block', html)
            self.assertIn('Drag legend to move all · Double-click to rename', html)
            self.assertIn('id="taxonomyLegendScale" type="range" min="50" max="200" step="5"', html)
            self.assertIn('["taxonomy", "taxonomy-legend"]', html)
            self.assertIn('state.taxonomyLegendScale', html)
            self.assertIn('data-phenotype-legend-block', html)
            self.assertIn('data-phenotype-legend-key', html)
            self.assertIn('Phenotype transitions', html)
            self.assertIn('id="phenotypeLegendScale" type="range" min="50" max="200" step="5"', html)
            self.assertIn('["phenotype", "phenotype-legend"]', html)
            self.assertIn('species-map-phenotype-legend-offset', html)
            self.assertIn('species-map-phenotype-legend-overrides', html)
            self.assertIn('openPhenotypeLegendEditor(', html)
            self.assertIn('enableLegendScales()', html)
            self.assertNotIn('const rightEdge = x + Math.max(240, maxWidth)', html)
            self.assertNotIn('cursorX += itemWidth', html)
            self.assertIn('label.textContent = text', html)
            self.assertNotIn('one color per clade', html)
            self.assertIn('width: min(700px, calc(100% - 24px))', html)
            self.assertIn('max-height: calc(100vh - 72px)', html)
            self.assertIn('grid-template-columns: 90px minmax(0, 1fr)', html)
            self.assertIn('flex-direction: column', html)
            self.assertIn('data-control-theme="purple"', html)
            self.assertIn('data-control-theme="black"', html)
            self.assertIn('data-control-theme="light"', html)
            self.assertIn('--cp-ink: #ffffff', html)
            self.assertIn('species-map-control-panel-theme', html)
            self.assertIn('applyControlTheme(localStorage.getItem', html)
            self.assertIn('--cp-bg: rgba(23, 18, 57, .50)', html)
            self.assertIn('background: linear-gradient(180deg, color-mix(in srgb, var(--cp-accent) 24%, transparent)', html)
            self.assertIn('--cp-surface: rgba(67, 42, 132, .72)', html)
            self.assertIn('background: var(--cp-bg)', html)
            self.assertIn('backdrop-filter: blur(2px) saturate(105%)', html)
            self.assertIn('.tree-control-window.minimized { width: min(700px, calc(100% - 24px)); }', html)
            self.assertIn('box-shadow: inset 3px 0 var(--cp-accent)', html)
            self.assertIn('border-radius: 2px; padding: 4px 7px', html)
            self.assertIn('#controlTabBasic:not([hidden]) { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); }', html)
            self.assertIn('.tree-control-window .tree-tools { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr));', html)
            self.assertIn('appearanceTitle.textContent = "Appearance"', html)
            self.assertIn('id="branchEventDisplaySelect"', html)
            self.assertIn('<option value="pie">Pie chart</option>', html)
            self.assertIn('id="eventPieScale" type="range" min="50" max="200" step="5"', html)
            self.assertIn('species-map-event-pie-scale', html)
            self.assertIn('enableEventPieScale()', html)
            self.assertIn('function eventPieRadius(branch)', html)
            self.assertIn('function eventPieSlicePath(', html)
            self.assertIn('return (4 + Math.max(0, Math.min(1, scaled)) * 8) * state.eventPieScale', html)
            self.assertIn('class: `event-pie-slice ${part.kind}', html)
            self.assertIn('species-map-branch-event-display', html)
            self.assertIn('role="tablist" aria-label="Control categories"', html)
            self.assertIn('data-control-tab="basic">Basic</button>', html)
            self.assertIn('data-control-tab="filters"', html)
            self.assertIn('data-control-tab="output"', html)
            self.assertIn('id="controlTabBasic" role="tabpanel"', html)
            self.assertIn('id="controlTabFilters" role="tabpanel"', html)
            self.assertIn('id="controlTabOutput" role="tabpanel"', html)
            self.assertIn('localStorage.setItem("species-map-control-tab", selected)', html)
            self.assertIn('$("controlTabFilters").append(', html)
            self.assertIn('legendsTitle.textContent = "Legends"', html)
            self.assertIn('legendControls.append($("taxonomyLegendScaleControl"), $("phenotypeLegendScaleControl"))', html)
            self.assertNotIn('<div class="legend"><span><i class="dot gain"></i>gain</span>', html)
            self.assertIn('.phenotype-gain { fill: #ff1744;', html)
            self.assertIn('.phenotype-loss { fill: #9aa1a8;', html)
            self.assertNotIn('id="restoreLabelDefaults"', html)
            self.assertIn('if (key.startsWith("species-map-")) localStorage.removeItem(key)', html)
            self.assertIn('window.location.reload()', html)
            self.assertIn('$("resetAllSettingsRow")', html)
            ...
            self.assertIn('.tree-control-window .filter-match-count { display: inline;', html)
