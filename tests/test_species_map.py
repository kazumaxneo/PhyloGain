...
class ProjectTests(unittest.TestCase):
    ...
    def test_build_project_with_existing_eggnog_annotations(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('species-map-taxonomy-legend-offsets', html)
            self.assertIn('openTaxonomyLegendEditor(', html)
            self.assertIn('lastLegendClickKey === interaction.key', html)
            self.assertIn('state.taxonomyLegendOffsets.set(', html)
            self.assertIn('Drag to move · Double-click to rename', html)
            ...
            self.assertIn('width: min(500px, calc(100% - 24px))', html)
            self.assertIn('max-height: min(84vh, 840px)', html)
            self.assertIn('padding: 7px 20px', html)
